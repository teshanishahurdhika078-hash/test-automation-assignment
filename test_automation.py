...
DEFAULT_EXCEL_CANDIDATES = [
    str(TESTS_DIR / "Assignment 1 - Test cases.xlsx IT23843066.xlsx"),
]
...
def _find_column_index(header_values: list, requested_name: str | None, candidates: list[str]) -> int | None:
    ...
    if requested_name:
        ...
        if n:
            if n in norm_to_index: return norm_to_index[n]
            for i, v in indexed:
                if n in _normalize_header(v) or _normalize_header(v) in n: return i

    # First pass: Exact match for any candidate
    for c in candidates:
        n = _normalize_header(c)
        if n and n in norm_to_index:
            return norm_to_index[n]
        for i, v in indexed:
            if n == _normalize_header(v):
                return i
    
    # Second pass: Substring match for any candidate
    for c in candidates:
        n = _normalize_header(c)
        if not n: continue
        for i, v in indexed:
